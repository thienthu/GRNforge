...
if __name__ == '__main__':
    ...
    if not os.path.exists(base_result_path+run_name+'/model_exp_exp/'):
        os.makedirs(base_result_path+run_name+'/model_exp_exp/')

    print(TG_models_exp_exp_scores)
    print(path_to_save)
    
    path_to_save = base_result_path+run_name+'/model_exp_exp/baseline_model_1_scores.pkl'
    with gzip.open(path_to_save, "wb") as file:  # 'wb' means write in binary mode
        pickle.dump(TG_models_exp_exp_scores, file)
    path_to_save = base_result_path+run_name+'/model_exp_exp/baseline_model_1_evals.pkl'
    with gzip.open(path_to_save, "wb") as file:  # 'wb' means write in binary mode
        pickle.dump(TG_models_exp_exp_evals, file)    
    path_to_save = base_result_path+run_name+'/model_exp_exp/baseline_model_1_predictive_performance.pkl'
    with gzip.open(path_to_save, "wb") as file:  # 'wb' means write in binary mode
