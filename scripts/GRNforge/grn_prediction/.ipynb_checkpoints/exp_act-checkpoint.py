...
if __name__ == '__main__':
    ...
    # select 
        ...
        # # train model
        model.trainModelCV()
        TG_models_exp_act_scores[target_gene]           = model.mapped_importance_scores
        TG_models_exp_act_evals[target_gene]            = model.evals_result
        TG_models_exp_act_pred_performance[target_gene] = (model.data_mean, model.train_mae, model.test_mae, model.test_mae_std, model.train_mae_std)
    b=datetime.utcnow()
    print(b-a)

    # Create the directory if it doesn't exist
    if not os.path.exists(base_result_path+run_name+'/model_exp_act/'):
        os.makedirs(base_result_path+run_name+'/model_exp_act/')
    
    path_to_save = base_result_path+run_name+'/model_exp_act/baseline_model_1_scores.pkl'
    with gzip.open(path_to_save, "wb") as file:  # 'wb' means write in binary mode
        pickle.dump(TG_models_exp_act_scores, file)
    path_to_save = base_result_path+run_name+'/model_exp_act/baseline_model_1_evals.pkl'
    with gzip.open(path_to_save, "wb") as file:  # 'wb' means write in binary mode
        pickle.dump(TG_models_exp_act_evals, file)    
    path_to_save = base_result_path+run_name+'/model_exp_act/baseline_model_1_predictive_performance.pkl'
    with gzip.open(path_to_save, "wb") as file:  # 'wb' means write in binary mode
        pickle.dump(TG_models_exp_act_pred_performance, file)    